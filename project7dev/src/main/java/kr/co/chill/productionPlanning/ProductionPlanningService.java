...
public interface ProductionPlanningService {
	...
	//선택조회(상세보기)
	ProductionPlanningDTO readProductionPlanningByPrplNo(int prpl_no) throws Exception;
	//제품코드로 검색조회
	...
	//등록 시 사원번호로 사원정보 조회
	ProductionPlanningDTO getEmployeeByEmpNo(int emp_no) throws Exception;
	//등록 시 창고번호로 창고조회
	ProductionPlanningDTO getPstorageByPstorageNo(int pstorage_no) throws Exception;
	//수정
	void updateProductionPlanning(ProductionPlanningDTO productionPlanningDTO) throws Exception;
	//삭제
	void deleteProductionPlanning(int prpl_no) throws Exception;
